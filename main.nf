...
workflow {
    // NF 26 strict-syntax dropped the `-entry` CLI flag; the recommended
    // migration is to dispatch named workflows from the default workflow
    // via a parameter (--entry NAME). The chunked driver invokes this with
    // --entry REPORT_ONLY at the end of a run and --entry SRA_DISCOVERY_ONLY
    // at the start.
    if (params.entry == 'SRA_DISCOVERY_ONLY') {
        SRA_DISCOVERY_ONLY()
    } else if (params.entry == 'REPORT_ONLY') {
        REPORT_ONLY()
    } else if (params.entry) {
        error "Unknown --entry value: '${params.entry}'. Valid: SRA_DISCOVERY_ONLY, REPORT_ONLY"
    } else {
